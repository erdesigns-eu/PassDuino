...
begin
  ...
  case frmAddPassword.Execute of
    ...
    begin
      ...
      begin
        WriteToCOMPort(Format('SET%sSTR%s%d%s%s', [COMDelimiter, COMDelimiter, I, COMDelimiter, edtPassword.Text]));
        T := WaitForResponse;
      end;
...
begin
  ...
  try
    ...
    begin
      ...
      Ini.WriteString('PassDuino', 'Password', ProfilePassword);
      Ini.WriteString('PassDuino', 'COMPort', ComPort.Port);
    end;
    with frmMain do
    begin
      for I := 0 to lvPasswords.Items.Count -1 do
...
begin
  ...
  try
    ...
    begin
      ...
      if ConnectToPassDuino then
      begin
        WriteToCOMPort(Format('SET%sINT%s%d%s%d', [COMDelimiter, COMDelimiter, 0, COMDelimiter, StrLength]));
        T := WaitForResponse;
        WriteToCOMPort(Format('SET%sINT%s%d%s%d', [COMDelimiter, COMDelimiter, 1, COMDelimiter, DefaultPassword]));
        T := WaitForResponse;
        WriteToCOMPort(Format('SET%sINT%s%d%s%d', [COMDelimiter, COMDelimiter, 2, COMDelimiter, B[PressReturn]]));
        T := WaitForResponse;
        WriteToCOMPort(Format('SET%sINT%s%d%s%d', [COMDelimiter, COMDelimiter, 3, COMDelimiter, TypeDelay]));
        T := WaitForResponse;
      end;
